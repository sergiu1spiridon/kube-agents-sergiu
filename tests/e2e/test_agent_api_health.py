...
def test_platform_agent_api_health_ping(
    ...
    payload = json.dumps({"model": "model-default", "input": "ping"}).encode("utf-8")
    headers = {
    ...
    try:
        ...
        pytest.fail(f"Platform Agent API health ping failed with HTTP {e.code}: {e.read().decode('utf-8')}")
